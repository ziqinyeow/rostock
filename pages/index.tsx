...
const Home: NextPage = ({
  ...
  const _stock = Object.fromEntries(
    Object.entries(stock).sort(
      // @ts-ignore
      ([, a], [, b]) => b?.[`trend_${viewDay}`] - a?.[`trend_${viewDay}`]
    )
  );
          ...
          <div className="w-full grid-cols-5 gap-6 mb-6 xl:gap-8 sm:grid">
            <div className="col-span-3 mb-10">
              ...
              {Object.keys(_stock)
                ?.slice(0, 5)
                .map((key, index) => (
                  <Link href={`/analytics/${key}`} key={key}>
                    <a>
                      <div className="relative group">
                        ...
                        <div className="relative p-5 mb-6 transition duration-200 bg-white border rounded-md group-hover:text-white group-hover:bg-gradient-to-r dark:group-hover:from-primary-400 dark:group-hover:to-primary-300 group-hover:from-primary-100 group-hover:to-primary-200 bg-gradient-to-r dark:bg-black dark:border-gray-700">
                          ...
                          <h1 className="mb-2">{key}</h1>
                          {/* <h3 className="mb-4 font-medium">
                          {stock[key]["STOCK CODE"]}: {stock[key]["ISIN CODE"]}
                        </h3> */}
                          <div className="flex">
                            <h4 className="pr-4">
                              Stock Trend Next {viewDay} Days:
                            </h4>
                            <h4>
                              {stock[key]?.[`trend_${viewDay}`] ? (
                                <span>
                                  {" "}
                                  {Math.round(
                          ...
                          <div className="flex">
                            <h4 className="pr-4">
                              Stock Average Next {viewDay} Days:
                            </h4>
                            <h4>
                              {stock[key]?.[`average_${viewDay}`] ? (
                                <span>
                                  RM{" "}
                                  {Math.round(
                                    (Number(
                          ...
                          <div className="flex">
                            <h4 className="pr-4">Lowest Price At:</h4>
                            <h4>
                          ...
                          <div className="flex">
                            <h4 className="pr-4">Highest Price At:</h4>
                            <h4>
                ...
                <div className="space-y-4">
                  <div className="p-4 border-2 rounded-md dark:border-gray-700">
                    <h5 className="font-medium">
                      <strong>/</strong> - Displaying Top 5 Stocks based on
                      Stock Trend
                    </h5>
                  </div>
                  <div className="p-4 border-2 rounded-md dark:border-gray-700">
                    <h5 className="font-medium">
                      <strong>/analytics</strong> - Displaying Top 22 Stocks
                      based on Stock Trend
                    </h5>
                  </div>
                  <div className="p-4 border-2 rounded-md dark:border-gray-700">
                    <h5 className="font-medium">
                      <strong>/analytics/[Stock Name]</strong> - Displaying the
                      relevant info based on the stock
                    </h5>
                  </div>
                  <div className="p-4 border-2 rounded-md dark:border-gray-700">
                    <h5 className="font-medium">
                      <strong>/predict</strong> - Predict the next stock price
                      based on user input
                    </h5>
                  </div>
                </div>
